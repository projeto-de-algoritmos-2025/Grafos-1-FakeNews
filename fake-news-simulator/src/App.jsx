...
function App() {
  ...
  // Initilize graphData and maps links to improve BFS complexity, basically an adjacency vector
  useEffect(() => {
  ...
  useEffect(() => {
    if (fgRef.current) {
      ...
      fgRef.current.d3Force('link').distance(link => linkWeightMap[link.type] * 100); 
    }
  }, []);
  ...
  const nodeColor = useCallback((node) => {
    return infectedNodes.hasOwnProperty(node.id) ? 'red' : 'lightgrey';
  }, [infectedNodes]);
    ...
    <>
      ...
      <div style={{ width: '100vw', height: '100vh'}}>
        <ForceGraph2D
          ...
          // Desenha o avatar primeiro
          ctx.drawImage(img, node.x - 10, node.y - 10, 20, 20);

          // Desenha o círculo colorido por cima do avatar, sem o clip
          ctx.beginPath();
